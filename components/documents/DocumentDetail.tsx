...
/* ══════════════════════════════════════════════════════════════════════ */
export default function DocumentDetail({ doc, project, ownerProfile, plan }: Props) {
    ...
    <>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-4">
          ...
          {/* CTA Signer */}
          {plan === 'en_famille' || plan === 'avec_avocat' ? (
            <SignCard doc={doc} isSigned={isSigned} />
          ) : (
...
/* ── Sign card (plan actif) ── */
function SignCard({ doc, isSigned }: { doc: DocRow; isSigned: boolean }) {
  ...
  async function handleSign() {
    setLoading(true)
    setError(null)
    const res = await fetch('/api/yousign/sign', {
      method:  'POST',
  ...
  if (isSigned) {
    return (
      <div className="bg-green-50 border border-green-200 rounded-2xl p-5 text-center">
        ...
        <p className="font-semibold text-green-800">Document signé</p>
        <p className="text-xs text-green-600 mt-1">Les deux parties ont signé électroniquement.</p>
      </div>
    )
  }
  ...
  if (isPending) {
    return (
      <div className="bg-amber-50 border border-amber-200 rounded-2xl p-5">
        <p className="font-semibold text-amber-800 mb-1">Signature en cours</p>
  ...
  return (
    <div className="bg-[#1C3B2E] rounded-2xl p-5 text-white">
      ...
      <p className="text-white/60 text-sm mb-4">
        Yousign envoie un email à chaque partie avec leur lien de signature sécurisé.
      </p>
      {error && (
        <p className="text-red-300 text-xs mb-3 bg-red-900/30 rounded-lg px-3 py-2">{error}</p>
      )}
      <button
        onClick={handleSign}
        disabled={loading}
        className="w-full flex items-center justify-center gap-2 px-5 py-3 rounded-xl bg-accent text-white font-semibold text-sm hover:bg-accent-dark transition-colors disabled:opacity-60"
      >
        {loading ? (
          <><Loader2 size={15} className="animate-spin" /> Envoi en cours…</>
        ) : (
          <><PenLine size={15} /> Lancer la signature</>
        )}
      </button>
      <p className="text-center mt-2 text-xs text-white/40">eIDAS · conforme droit français</p>
    </div>
  )
}
